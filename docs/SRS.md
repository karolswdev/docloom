...
| <a name="ARCH-008"></a>**ARCH-008** | Decoupled Agent Executor | The agent execution mechanism **MUST** be decoupled from the agent's internal logic. The core system's responsibility is to parse the agent's YAML definition and execute it as an external process, passing the required context (source path, output path, parameters). | Ensures the core `docloom` application remains agnostic to the specific technologies used by an agent (e.g., .NET, Python, shell scripts), maximizing extensibility.
| <a name="ARCH-009"></a>**ARCH-009** | Agent Registry & Discovery | The system **MUST** implement an agent registry that discovers agent definition files from predefined locations (e.g., `$HOME/.docloom/agents`, `<project>/.docloom/agents`). | Provides a consistent and predictable mechanism for managing and locating agents, supporting both user-global and project-specific configurations.
| <a name="ARCH-010"></a>**ARCH-010** | Intermediate Artifact Cache | The system **MUST** manage a temporary cache directory for the artifacts generated by agents. This directory **MUST** be automatically cleaned up or managed by the system. | Prevents cluttering the user's source directory with intermediate files and simplifies the data pipeline between the agent and the generator.
---
